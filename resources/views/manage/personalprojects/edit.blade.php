...
<div class="flex justify-between items-center mb-6">
    <h1 class="text-4xl mb-6">Edit {{ $project->name }}</h1>
        <form action="{{ route('manage.personalprojects.destroy', $project->id) }}" class="inline" method="POST">
            @csrf
            @method('DELETE')
    ...
    <form action="{{ route('manage.personalprojects.update', $project->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
